...
- 首页按钮重新设计
- 首页添加卡牌元素
- 新增三槽存档系统：独立保存金币与升级进度，并在切换时自动载入对应数据
- 首页新增「SAVES」按钮，可随时打开存档面板并选择当前使用的槽位
##### **Refactor**
- 重构敌人 AI：集中在 `aiService` 内实现多层感知 / 评估 / 决策逻辑，并根据环境卡、惩罚卡与道具卡智能选择摸牌、停牌和使用道具
##### **Fix**
- 只有 level 1 才会发放初始 item cards，之后的战斗不会发放；只会给敌人在每次战斗开始发放初始 item cards
- 修复敌人使用道具后未正确轮到玩家回合的问题
#### [2025-11-26]
##### **Feature**
- 教程页面（2 页）
##### **Refactor**
- 重构一：
  - 新增 `engine/services/runLifecycleService.ts`，统一 `startNewRun` / `prepareNextLevel` 与 GameState 构造（敌人、环境、惩罚卡、套牌、环境规则一次完成）
...
- 重构二：
  ...
  - 抽离 `CreateMetaFn` 至 `engine/services/serviceTypes.ts`，便于多个服务共用统一的日志工厂
- 重构三：
...
- 添加 9 张环境卡
  ...
  - Perfect 奖励：当前对战中，任意玩家在结算时打出 perfect（等于当前 target），抽一张道具卡
...
#### [2025-11-24]
##### **Feature**
- 新增 Penalty Cards 惩罚卡系统：每场 Battle 固定抽取 1 张惩罚卡，由数据驱动的 damageFunction 统一处理胜负双方 HP 结算
...
- 新道具卡
  ...
  - 按当前持有道具数等量回复生命
##### **Refactor**
- 拆分 `combatSerivce` 与 `aiService`、`itemEffectService` 与 `roundService`
#### [2025-11-23]
##### **Feature**
